...
public class RedisUtil {
     ...
     * @return
     */
    public String acquireLockWithTimeout(String lockKey,Long lockExpire) {
        //锁的值
        String identifier = UUID.randomUUID().toString();
        if (redisTemplate.opsForValue().setIfAbsent(lockKey, identifier)) {
            //设置锁的过期时间
     ...
     * @param consumer
     */
    public void executePipelined(Consumer<RedisConnection> consumer) {
     ...
     * @return
     */
    public Set<Object> zRange(String key, double maxScore, long count) {
        return  redisTemplate.opsForZSet().rangeByScore(key, 0d , maxScore, 0,  count);
    }
     ...
     * @return
     */
    public boolean zDel(String key, String itemKey) {
        Long count =  redisTemplate.opsForZSet().remove(key, itemKey);
     ...
     * @return
     */
    public boolean zDels(String key, String[] itemKeys) {
        Long count =  redisTemplate.opsForZSet().remove(key, itemKeys);
        return count != null && count > 0;
    }
     ...
     * @return 对应的多个键值
     */
    public boolean hPutIfAbsent(String key, Object itemKey, Object value) {
        return redisTemplate.opsForHash().putIfAbsent(key, itemKey, value);
    }
